...
function setup(){ 
  ...
  capture.hide(); 
  for(let i=0;i<img.length;i++){///////////////// 36, 이미지 x,y random(-width/2,width/2),random(-height/2,height/2)로 수정 
    images[i]=new Img(i, random(-width/2,width/2),random(-height/2,height/2),random(-1,1),random(-0.05,0.05),random(-2,2),random(-2,2));
  }  
...
function draw(){
  ...
  for(let i=0;i<sg.length;i++){//////////////////////// 54~56
    sg[i].draw();
  }
...
class Img {
  ...
  move(){
    ...
    if(this.x>width/2-img[this.i].width/2){ ////////////////////// 수정. 원래 있던 코드 삭제하고 밑에 네 개 추가, 123~138
